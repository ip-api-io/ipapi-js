...
|---|---|---|
| `score` | `number` | Risk score, 0 (safe) – 100 (high risk) |
...
| `factors` | `RiskScoreFactors` | `ip_factors` and/or `email_factors` |
...
Returns the underlying reputation data for an IP as a plain object (`Record<string,
unknown>`) — use it when you want the source signals rather than a computed score.
...
- [Full tutorial on ip-api.io](https://ip-api.io/docs/sdk/javascript/fraud-risk-scoring)
